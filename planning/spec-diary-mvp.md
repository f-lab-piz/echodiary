...
- 관리자 공급자 전환 시점에 생성 요청이 동시에 들어오는 경우.
- 선택된 공급자에서 특정 모델 타입(`text-large` 또는 `image`)이 일시 장애인 경우.
...
- 서버는 공급자 후보를 `OpenAI`, `Gemini` 두 가지로만 지원한다.
- 관리자 계정은 서버 전역 LLM 공급자 1개를 선택할 수 있으며, 선택된 공급자만 전체 요청 처리에 사용된다.
- 모델 타입은 공통 논리 이름 `text-small`, `text-large`, `image`를 사용하고, 공급자별 실제 모델명은 매핑 테이블로 관리한다.
- 일기 초안 생성 기본 경로는 `text-small`을 사용하고, 품질 향상이 필요한 재생성/확장 요청은 `text-large`를 사용한다.
- 페르소나 대화 생성은 기본적으로 `text-large`를 사용하되, 비용 제한 모드에서는 `text-small`로 강등 가능해야 한다.
- 페르소나 이미지 생성은 `image` 타입 모델을 사용하며, 실패 시 자동 재시도 정책을 동일하게 적용한다.
- 관리자 공급자 변경은 저장 이후 `신규 요청`부터 반영하고, 이미 실행 중인 요청은 기존 공급자로 완료 처리한다.
- 선택 공급자 장애 시 요청은 즉시 실패 처리하고 재시도 경로를 제공한다(자동 타 공급자 폴백은 MVP 비포함).
...
- 공급자 전역 스위치 및 모델 매핑 테이블 운영.
- 낮은 비용 대안:
- 초기에는 페르소나를 고정 템플릿만 제공하고 이미지 생성을 제외해도 MVP 가치를 일부 검증할 수 있다.
- 초기에는 단일 공급자(OpenAI 또는 Gemini 중 1개)로 고정 운영해도 핵심 가설 검증이 가능하다.
- 비교 결과:
- 대안은 구현 비용이 낮지만 차별화(개인화/몰입감)와 장기 유지율 가설 검증력이 떨어진다.
- 단일 공급자 고정 대안은 운영 단순성은 높지만 장애/비용 변동 대응력이 낮다.
...
- Risk: 공급자 전환 직후 모델 응답 품질/형식 차이로 결과 일관성 저하.
- Mitigation: 공통 프롬프트 스키마 + 공급자별 출력 정규화 레이어를 적용한다.
- Risk: 공급자 장애 시 서비스 가용성 저하.
- Mitigation: 장애 탐지 시 빠른 실패와 명시적 재시도 UX를 제공하고, 운영자가 전역 공급자를 즉시 전환할 수 있게 한다.
...
- 모델 운영 정책은 `관리자 전역 공급자 선택(OpenAI/Gemini)`으로 채택하며, 공통 모델 타입(`text-small`, `text-large`, `image`)을 기준으로 라우팅한다.
...
- 관리자 전역 공급자 선택 UI/설정(OpenAI/Gemini) 및 서버 라우팅 반영.
- 공급자별 모델 매핑(`text-small`, `text-large`, `image`)과 요청 라우팅 로직.
- 비포함(Non-goal):
- 일기 내용 기반 장기 추세 기록/시각화.
- 주간/월간/연간 인사이트 리포트 생성.
- 페르소나 추천 자동화 및 고급 AB 테스트.
- 공급자 장애 시 자동 다중 공급자 폴백.
...
- 운영 설정 스키마 작성: `provider`, `text_small_model`, `text_large_model`, `image_model`.
- 공급자 전환 운영 가이드 작성: 반영 시점, 롤백 절차, 장애 대응.
